    ...
    <div>
      {showPopup && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[60]">
          <div className="bg-gradient-to-b from-[#0A0A0A] to-[#1A1A1A] p-5 rounded-xl relative w-full max-w-[400px] mx-auto shadow-2xl border border-gray-800">
            <button
              onClick={() => setShowPopup(false)}
              className="absolute -top-2 -right-2 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center hover:bg-red-600 transition-colors shadow-lg text-sm"
            >
              ✕
            </button>
            
            <div className="text-center space-y-3">
              <h2 className="text-2xl font-bold text-white font-aleo tracking-wide">GET READY</h2>
              <div className="space-y-0.5">
                <p className="text-lg text-red-500 italic font-medium">Event</p>
                <p className="text-lg text-red-500 italic font-medium">Coming Soon</p>
              </div>
              
              <div className="mt-3">
                <img 
                  src="/event-poster.jpg" 
                  alt="FutureTech Edge Event" 
                  className="w-full rounded-lg shadow-xl max-h-[300px] object-contain mx-auto"
                />
              </div>

              <button
                onClick={() => setShowPopup(false)}
                className="bg-red-500 hover:bg-red-600 text-white px-5 py-1.5 rounded-lg text-sm font-medium transition-colors shadow-lg mt-2"
              >
                View Details
              </button>
